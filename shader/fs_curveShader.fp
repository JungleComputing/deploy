...
void main (void)
{    
    vec2 rtex = vec2(abs(tCoords.x),abs(tCoords.y));
    vec3 c = ColorStatic.rgb;

    float alpha = 0.0;
    float enable = 1.0;
    
    if((tCoords.x == 0.0) && (tCoords.y == 0.0)) {
         alpha = Alpha;
    }
    else if((tCoords.x >= 5.0)) {
        vec2 dfx = dFdx(tCoords);
        vec2 dfy = dFdy(tCoords);
    ...
    else if ((tCoords.x > 0.0) && (rtex.y > 0.0 || rtex.x == 1.0)) {
        rtex.y -= 0.1;
          
        if(rtex.y < 0.0 && tCoords.y < 0.0) {
            // discard; // freezes NV tegra2 compiler
            alpha = 0.0;
        } else {
            ...
            float a = clamp(0.5 - ( position/length(f) ) * sign(tCoords.y), 0.0, 1.0);
            alpha = Alpha * a;
        }
    }
