...
public class GoggleSwing {
	...
	public static Box sliderBox(String label, ChangeListener listener, int min, int max, int spacing, int norm, JLabel dynamicLabel) {
	...
	private static Box vBoxedComponents(ArrayList<Component> components, boolean bordered) {		
		Box hrzBox = Box.createHorizontalBox();
			hrzBox.add(horizontalStrut(2));
				...
				if (bordered) vrtBox.setBorder(new BevelBorder(BevelBorder.RAISED));
