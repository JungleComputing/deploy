...
public abstract class Element implements
		ibis.deploy.monitoring.collection.Element {
	@SuppressWarnings("unused")
	private static final Logger logger = LoggerFactory
			.getLogger("ibis.deploy.monitoring.collection.impl.Element");
	...
	// getters
	public ibis.deploy.monitoring.collection.Metric getMetric(
			ibis.deploy.monitoring.collection.MetricDescription desc)
	...
	public ibis.deploy.monitoring.collection.Link getLink(
			ibis.deploy.monitoring.collection.Element destination)
	...
	public void addLink(ibis.deploy.monitoring.collection.Element destination,
			ibis.deploy.monitoring.collection.Link link) {
	...
	public ibis.deploy.monitoring.collection.Link[] getLinks() {
		return links.values().toArray(
				new ibis.deploy.monitoring.collection.Link[0]);
	}
	...
	public void addMetric(
			ibis.deploy.monitoring.collection.MetricDescription description) {
		if (!metrics.containsKey(description)) {
			Metric newMetric = (Metric) ((MetricDescription) description)
					.getMetric(this);
