...
public class MPIBytesReceivedPerSecond extends
    ...
    private static final Logger logger = LoggerFactory
            .getLogger(MPIBytesReceivedPerSecond.class);

    // private static final float MAX = 1024000; // 1GB

    public MPIBytesReceivedPerSecond() {
        super();

        name = "MPI_Bytes_Received_Per_Sec";
            ...
            for (Map.Entry<?, ?> incoming : ((Map<?, ?>) results[0]).entrySet()) {
                if (incoming.getKey() instanceof IbisIdentifier
                    ...
                    if (time_elapsed != 0) {
                        ...
                        float percent = perSec / maxForPercent; // (float)max;
                        if (percent < 0f)
                            percent = 0f;
                        if (percent > 1f)
                            percent = 1f;
