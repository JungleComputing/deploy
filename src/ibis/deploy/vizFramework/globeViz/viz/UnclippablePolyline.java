    ...
    protected void drawOrderedRenderable(DrawContext dc) {
        ...
        try {
            ...
            if (!dc.isPickingMode()) {
                ...
                dc.getGL()
                        .getGL2()
                        .glColor4ub((byte) this.color.getRed(),
                                (byte) this.color.getGreen(),
                                (byte) this.color.getBlue(),
                                (byte) this.color.getAlpha());
            } else {
                // We cannot depend on the layer to set a pick color for us
                // because this Polyline is picked during ordered
                // rendering. Therefore we set the pick color ourselves.
                Color pickColor = dc.getUniquePickColor();
                Object userObject = this.getDelegateOwner() != null ? this
                        .getDelegateOwner() : this;
                this.pickSupport.addPickableObject(pickColor.getRGB(),
                        userObject, null);
            ...
            Matrix projection = Matrix.fromPerspective(view.getFieldOfView(),
                    view.getViewport().getWidth(), view.getViewport()
                            .getHeight(),
                    // view.getNearClipDistance(), view.getFarClipDistance());
                    view.getNearClipDistance(), 1.08 * view
                            .getFarClipDistance());
            ...
            // // create quadratic object for cylinders
            ...
            // // glu.gluCylinder(cylinder, 100, 100, 100, 20, 2);
            //
            // // gl.glColor3f(.5f, .5f, .5f);
            // // gl.glPushMatrix();
            // // gl.glTranslated(p.x, p.y, p.z);
            // // glu.gluCylinder(cylinder, 10, 10, 10, 20, 2);
            // // gl.glPopMatrix();
            // // }
            // }
            ...
            for (ArrayList<Vec4> span : this.currentSpans) {
                ...
                // Since segements can very often be very short -- two vertice
                // -- use explicit rendering. The
                // overhead of batched rendering, e.g., gl.glDrawArrays, is too
                // high because it requires copying
                // the vertices into a DoubleBuffer, and DoubleBuffer creation
            ...
            if (this.highlighted) {
                if (!dc.isPickingMode()) {
                    ...
                    dc.getGL()
                            .getGL2()
                            .glColor4ub((byte) this.highlightColor.getRed(),
