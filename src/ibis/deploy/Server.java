...
public class Server implements Runnable, MetricListener {
	...
	public Server(File[] libs, Cluster cluster, boolean hubOnly)
		...
		if (resourceBrokerURI == null) {
			throw new Exception("no resource broker URI given for cluster " + cluster);
		}
	...
	private static String classpathFor(File file, String prefix) {
		logger.debug("getting classpath for " + file + " with prefix " + prefix);
		...
		String result = prefix + file.getName()
				+ File.separator + "*" + File.pathSeparator + prefix
			 + file.getName() + File.pathSeparator;
		for (File child : file.listFiles()) {
			result = result + classpathFor(child, file.getName() + File.separator);
		}
		return result;
	}
