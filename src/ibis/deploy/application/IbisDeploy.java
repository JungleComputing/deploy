...
public class IbisDeploy {
    ...
    public static void main(String[] args) {
        ...
        List<Job> jobs = null;
        try {
            jobs = deployer.loadRun(properties);
        } catch (Exception e) {
            System.err.println("failed to load jobs");
        ...
        while (true) {
            ...
            for (Job job : jobs) {
                allJobsDone = allJobsDone
                        && (job.getStatus().get(JobState.STOPPED) + job
                                .getStatus().get(JobState.SUBMISSION_ERROR)) == job
                                .getSubJobs().size();
            }
            if (allJobsDone) {
                break;
