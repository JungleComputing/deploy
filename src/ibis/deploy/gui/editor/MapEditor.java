...
public class MapEditor implements PropertyChangeListener, ChangeableField 
{

    private static final GeoPosition DEFAULT_GEO_POSITION = new GeoPosition(
    		MapUtilities.localClusterLatitude, MapUtilities.localClusterLongitude);
     ...
     * @param latitude - initial value for latitude
     * @param longitude - initial value for longitude
     */
    		...
    		double latitude, double longitude) 
    {
        ...
        latitudeEditor = new GeoPositionTextEditor(container, "Latitude: ", latitude, true);
        ...
        longitudeEditor = new GeoPositionTextEditor(container, "Longitude: ", longitude, true);
        ...
        mapButton.addActionListener(new ActionListener() 
        {
			...
			{
				if(mapDialog == null)
					mapDialog = new LocationPickerDialog((JFrame)form.getRootPane().getParent(), true, 
							latitudeEditor.getValue(), longitudeEditor.getValue());
				else
					mapDialog.display(true, latitudeEditor.getValue(), longitudeEditor.getValue()); 
			}
		});
     ...
     * @param number - the value to be truncated
    ...
    /**
     * @return - true if the value in any of the editors is not the initial value
     */
    @Override
    ...
    class LocationPickerDialog extends JDialog implements ActionListener, PropertyChangeListener
    {
        /**
    	 * Dialog that allows the user to select locations directly on the map or 
    	 * by providing values in the editors. Values in the editors reflect on the
    	 * map an vice-versa
    	 */
         ...
         * @param modal - created dialog will be modal / non-modal 
         * @param latitude - initial value for latitude
         * @param longitude - initial value for longitude
         */
        public LocationPickerDialog(JFrame frame, boolean modal, Object latitude, Object longitude) 
        {
            super(frame, modal);
            ...
            if (latitude == null && longitude == null) 
                ...
                position = new GeoPosition(Double.parseDouble(latitude.toString()), 
            ...
            dialogLatitudeEditor = new GeoPositionTextEditor(longLatPanel, "Latitude:", 
            		position.getLatitude(), false);
            ...
            dialogLongitudeEditor = new GeoPositionTextEditor(longLatPanel, "Longitude:", 
            ...
            mapKit.getMainMap().addMouseListener(new MouseListener() {
            	...
            	/**
            	 * when the location on the map is updated, the values of the editors
            	 * are also updated
            	 */
                public void mouseClicked(MouseEvent e) 
                {
                    GeoPosition oldPosition = mapKit.getMainMap().getCenterPosition();
                    GeoPosition clusterLocation = mapKit.getMainMap().convertPointToGeoPosition(
                            e.getPoint());
                    mapKit.getMainMap().setAddressLocation(clusterLocation);
                    
                    dialogLatitudeEditor.setValue(truncate(clusterLocation.getLatitude()));
                    dialogLongitudeEditor.setValue(truncate(clusterLocation.getLongitude())); 
    	...
    	{
    		GeoPosition position = new GeoPosition(Double.parseDouble(latitude.toString()), 
    				Double.parseDouble(longitude.toString()));
    		mapKit.getMainMap().setAddressLocation(position);
    		
    		//set zoom level to the maximum level
    		mapKit.setZoom(mapKit.getMainMap().getTileFactory().getInfo().getMaximumZoomLevel());
    	...
    	/**
    	 * Listener for the Save / Cancel buttons.
    	 * On save, transfer the values to the MapEditor editors and close dialog.
        ...
        /**
         * Property change listener for the dialog editors.
         * Changes in latitude / longitude will also be visible on the map
         */
		public void propertyChange(PropertyChangeEvent event) 
		{
			if(event.getSource().equals(dialogLatitudeEditor.getTextField()))
			{
				position = new GeoPosition((Double) dialogLatitudeEditor.getValue(), 
    ...
    class DialogMap extends JXMapKit
    {
    	...
    	public DialogMap(GeoPosition position)
    	{
            ...
            getMainMap().setZoom(getMainMap().getTileFactory().getInfo().getMaximumZoomLevel());
