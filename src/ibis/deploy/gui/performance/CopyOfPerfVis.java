...
public class CopyOfPerfVis  {
	...
	CopyOfPerfVis (GUI gui, GLCanvas canvas) {
		this.gui = gui;
		glu = new GLU();
		this.canvas = canvas;
	...
	public void setScope(int scope) throws ModeUnknownException {
		if (scope != CopyOfPerfVis.SCOPE_GRID && scope != CopyOfPerfVis.SCOPE_NODES) {
	...
	public void setZoom(int zoom) throws ModeUnknownException {
		if (zoom != CopyOfPerfVis.ZOOM_POOLS && zoom != CopyOfPerfVis.ZOOM_SITES && zoom != CopyOfPerfVis.ZOOM_NODES) {
			throw new ModeUnknownException();
		}	
	...
	public void setStat(int stat) throws ModeUnknownException {
		if (stat != CopyOfPerfVis.STAT_ALL && stat != CopyOfPerfVis.STAT_CPU && stat != CopyOfPerfVis.STAT_MEM) {
			throw new ModeUnknownException();
		}
			...
			for (Map.Entry<String, Integer> entry : newSizes.entrySet()) {				
	            ...
	            if (poolSize > 0) {
		            if (!poolSizes.containsKey(poolName) || poolSize != poolSizes.get(poolName)) {		            	
		            	pools.put(poolName, new Pool(this, poolName));		            	
		            }		            
		            pools.get(poolName).update(gui);
	...
	private void drawUniverse(GL gl, int mode) {		
		...
		for (Map.Entry<String, Pool> entry : pools.entrySet()) {		
			...
			pools.get(entry.getKey()).drawThis(gl, 0.25f, 1.0f, currentCollectionForm, currentElementForm, mode);
			i++;
		}
	}
