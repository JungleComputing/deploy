...
public class Vsite extends Vobject implements VobjectInterface {	
	...
	private int siteForm;
		
	public Vsite(PerfVis perfvis, Float[][][] nodeColors) {
		super(perfvis);
		
		nodes = new Vnode[nodeColors.length];
		
		for (int i = 0; i < nodeColors.length; i++) {
			nodes[i] = new Vnode(perfvis, nodeColors[i]);
		}			
	}

	public void setForm(int siteForm, int nodeForm, int statForm) throws ModeUnknownException {
		...
		for (int i = 0; i < nodes.length; i++) {
			nodes[i].setForm(nodeForm, statForm);
		}
	...
	public void setSize(float width, float height) {
		...
		this.scaleY = height;
		for (int i = 0; i < nodes.length; i++) {
			nodes[i].setSize(width, height);
		}
		
		if (siteForm == Vnode.CITYSCAPE) {
			int horz = (int)(Math.ceil(Math.sqrt(nodes.length))*(scaleXZ+0.1f));
			int vert = (int)scaleY;
			int dept = (int)(Math.ceil(Math.sqrt(nodes.length))*(scaleXZ+0.1f));
	...
	public void drawThis(GL gl, int glMode) {
		...
		if (siteForm == Vsite.CITYSCAPE) {
			drawCityscape(gl, glMode);
		} else if (siteForm == Vsite.CIRCLE) {
	...
	protected void drawCityscape(GL gl, int glMode) {		
		...
		int rows 		= (int)Math.ceil(Math.sqrt(nodes.length));
		int columns 	= (int)Math.floor(Math.sqrt(nodes.length));
		...
		for (int i=0; i < nodes.length; i++) {
			...
			//Setup the form
			try {
				nodes[i].setLocation(location);				
				nodes[i].setRelativeX(-(scaleXZ+separation)*row);
				nodes[i].setRelativeZ( (scaleXZ+separation)*column);
