...
//		ppp = new CubeNode(program, material, center.add(new Vec3(       0f,       0f,       0f)), size);
//		ppn = new CubeNode(program, material, center.add(new Vec3(       0f,       0f,-cubeSize)), size);
//		pnp = new CubeNode(program, material, center.add(new Vec3(       0f,-cubeSize,       0f)), size);
//		pnn = new CubeNode(program, material, center.add(new Vec3(       0f,-cubeSize,-cubeSize)), size);
//		npp = new CubeNode(program, material, center.add(new Vec3(-cubeSize,       0f,       0f)), size);
//		npn = new CubeNode(program, material, center.add(new Vec3(-cubeSize,       0f,-cubeSize)), size);
//		nnp = new CubeNode(program, material, center.add(new Vec3(-cubeSize,-cubeSize,       0f)), size);
//		nnn = new CubeNode(program, material, center.add(new Vec3(-cubeSize,-cubeSize,-cubeSize)), size);
	...
	public void addStar(int currentFrame, Particle star) {
		Vec3 location = star.location.get(currentFrame);
		if ((location.get(0) > center.get(0)-cubeSize) &&
			...
			if (subdivided) {
				addStarSubdivided(currentFrame, star);
			} else {
				stars.add(star);			
			}
		}
...
//		float u = (float) (0.5 * Math.log10((total_u / (float) childCounter))) -5f;//
...
//		System.out.println(u);
	...
	public void doneAddingStars(int currentFrame) {
		if (subdivided) {
			ppp.doneAddingStars(currentFrame);
			ppn.doneAddingStars(currentFrame);
			pnp.doneAddingStars(currentFrame);
			pnn.doneAddingStars(currentFrame);
			npp.doneAddingStars(currentFrame);
			npn.doneAddingStars(currentFrame);
			nnp.doneAddingStars(currentFrame);
			nnn.doneAddingStars(currentFrame);
		}
		
		double totalLuminosity = 0;
		for (Particle star : stars) {
			totalLuminosity += star.getLastUniqueLuminosity(currentFrame);
		}
		
		Vec4 newColor = new Vec4();
		for (Particle star : stars) {
			double luminosity = star.getLastUniqueLuminosity(currentFrame);		
			Vec4 color = star.getLastUniqueColor(currentFrame);
			newColor = newColor.add(color.mul(luminosity/totalLuminosity));
		}		
	...
	public void addStarSubdivided(int currentFrame, Particle star) {
		Vec3 location = star.location.get(currentFrame);
		if (location.get(0) < center.get(0)) {
			if (location.get(1) < center.get(1)) {
				if (location.get(2) < center.get(2)) {
					nnn.addStar(currentFrame, star);
				} else {
					nnp.addStar(currentFrame, star);
				}
			} else {
				if (location.get(2) < center.get(2)) {
					npn.addStar(currentFrame, star);
				} else {
					npp.addStar(currentFrame, star);
				}
			}
		} else {
			if (location.get(1) < center.get(1)) {
				if (location.get(2) < center.get(2)) {
					pnn.addStar(currentFrame, star);
				} else {
					pnp.addStar(currentFrame, star);
				}
			} else {
				if (location.get(2) < center.get(2)) {
					ppn.addStar(currentFrame, star);
				} else {
					ppp.addStar(currentFrame, star);
				}
			}
		}
	}
	...
	public void draw(GL3 gl, Mat4 MVMatrix) {
		if (initialized) {
//			Mat4 newM = new Mat4(MVPMatrix);
//			newM = newM.mul(SMatrix);
//			Mat4 newM = MVPMatrix.mul(TMatrix);
