...
public class Board implements Serializable {
    ...
    public String toString() {
        if (currentPlayerIsBlack) {
            return "Board, next=black, turn=" + turn + ", winner=" + winner;
        } else {
            return "Board, next=white, turn=" + turn + ", winner=" + winner;
        }
    }
    ...
    public Move bestMove() {
        ...
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                if (isEmpty(x, y)) {
                    ...
                    Move childBestMove = child.bestMove();

                    if (childBestMove
                            .isBetter(bestMove, currentPlayerIsBlack())) {
        ...
        Move result = new Move(bestX, bestY, bestMove.blackWins(), bestMove
                .getMaxTurns() + 1);
        return result;
    }

    /**
     * Leaf jobs are jobs with "size" number of turns, or a winner
     * 
     * @return true if this board belongs to a leaf job.
     */
    public boolean isLeaf() {
        return turn > (size + 1) || winner != UNDETERMINED;
    }

    Board[] getChildren() {
        // check if board is an end board
        if (isLeaf()) {
            logger.error("tried to get children of a leaf job!",
                    new Exception());
            System.exit(1);
        }
        ...
        for (int x = 0; x < size; x++) {
            for (int y = 0; y < size; y++) {
                if (isEmpty(x, y)) {
                    // place a stone
                    try {
                        result.add(new Board(this, x, y));
                    } catch (Exception e) {
                        logger.error("could not place stone,"
                                + " but space should be empty");
                        System.exit(1);
                    }
                }
        ...
        return result.toArray(new Board[0]);
    }

    public Move bestMove(Board[] children, Move[] results) {
        int bestX = -1;
        int bestY = -1;
        Move bestMove = null;
        ...
        for (int i = 0; i < children.length; i++) {
            if (results[i] == null) {
                logger.error("EEP! result null", new Exception());
                System.exit(1);
            }

            Move childBestMove = results[i];

            if (childBestMove.isBetter(bestMove, currentPlayerIsBlack())) {
                bestMove = childBestMove;
                bestX = children[i].lastX;
                bestY = children[i].lastY;
            }
        }
        Move result = new Move(bestX, bestY, bestMove.blackWins(), bestMove
                .getMaxTurns() + 1);
